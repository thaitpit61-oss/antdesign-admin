...
const ProductList: React.FC = () => {
  ...
  const columns: ProColumns<Product>[] = useMemo(
    () => [
      { title: 'STT', dataIndex: 'stt', search: false, width: 70, fixed: 'left' },

      {
        ...
        render: (_, record) => {
          ...
          const color = s === 'available' ? 'green' : s === 'sold' ? 'red' : 'default';
      ...
      { title: 'Chiết khấu', dataIndex: 'chietKhau', search: false, width: 110 },
      { title: 'Tổng sau CK', dataIndex: 'tongSauCK', search: false, width: 150 },

      {
        ...
        width: 120,
        render: (_, record) => [
          <a key="view" onClick={() => history.push(`/product/${record.maCan}`)}>
            Xem
          </a>,
        ],
      },
  ...
  return (
    <PageContainer title="Danh sách sản phẩm">
      <ProTable<Product>
        ...
        scroll={{ x: 1200 }}
        toolBarRender={() => [
          <Button key="create" type="primary" onClick={() => history.push('/product/create')}>
            Thêm sản phẩm
          </Button>,
        ]}
      />
